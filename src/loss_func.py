...
def BCELoss(inputs, targets):
    ...
    gt_index = torch.max(targets, dim=-1).indices
    inputs_s = torch.softmax(inputs,dim=-1)
    loss = F.nll_loss(
        torch.log(inputs_s),
        gt_index,
        reduction='sum')
    inputs = inputs.T
    targets = targets.T
    inputs_t = torch.softmax(inputs, dim=-1)
    gt_index = torch.max(targets, dim=-1).indices
    # inputs = inputs +1e-8
    loss += F.nll_loss(
        torch.log(inputs_t),
...
class PermutationLossHung(nn.Module):
    ...
    def forward(self, pred_dsmat: Tensor, gt_perm: Tensor, src_ns: Tensor, tgt_ns: Tensor) -> Tensor:
        ...
        # ali_perm[ali_perm >= 1.0] = 0.05 # Hung
        # ali_perm = ali_perm + 0.95
        pred_dsmat = torch.mul(ali_perm, pred_dsmat)
        gt_perm = torch.mul(ali_perm, gt_perm)
