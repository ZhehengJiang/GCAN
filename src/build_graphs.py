...
def build_graphs(P_np: np.ndarray, n: int, n_pad: int=None, edge_pad: int=None, stg: str='fc', sym: bool=True,
    ...
    if stg == 'tri':
        ...
        # add tree graph with center as root
        # all_points = P_np[0:n, :]
        # center = np.mean(all_points,axis=0)
        # points_dis = np.sqrt(np.sum(np.square(all_points - center), axis=1))
        # index = np.argsort(points_dis)
        # k_nearest = index[0:3]
        # for i in k_nearest:
        #     A[i, :]=1
        #     A[:,i]=1
        #     A[i,i]=0
        # find k nearest points of the center
        #tree graph with A[0,0] as root
        # A = np.zeros((P_np.shape[0],P_np.shape[0]))
        # A[0,1:]=1
        # A[1:,0]=1
    elif stg == 'near':
